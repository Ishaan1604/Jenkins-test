node {
    withCredentials([string(credentialsId: 'MONGO_URI', variable: 'MONGO_URI'), string(credentialsId: 'JWT_SECRET', variable: 'JWT_SECRET'), string(credentialsId: 'SENDGRID_API_KEY', variable: 'SENDGRID_API_KEY')]){
        ...
        stage("Start server") {
            ...
            node_image.pull();
            def container = node_image.run("-v /tmp/.npm:/.npm -e MONGO_URI=mongodb+srv://ishaaniscool:CMSgotDatabase@cms.fubtlod.mongodb.net/CMS?retryWrites=true&w=majority&appName=CMS -e JWT_SECRET=$JWT_SECRET -e SENDGRID_API_KEY=$SENDGRID_API_KEY -d") 
            container.inside {
                sh "npm install"
                sh "nohup npm start &"
                sleep(30)
